    ...
    with connection.cursor() as cursor:
        cursor.execute(f"""
            ...
            FROM (
                ...
                LEFT JOIN judge_contest ON (judge_submission.contest_object_id = judge_contest.id)
                WHERE (judge_problem.is_public AND
                       NOT judge_problem.is_organization_private AND
                       judge_submission.points IS NOT NULL AND
                       judge_submission.user_id = %s AND
                       (judge_contest.is_offline IS NULL OR NOT judge_contest.is_offline))
