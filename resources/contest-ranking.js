...
(function ($) {
    ...
    // ─── Hidden results (blind round / not-yet-unfrozen) ─────────────────────
    // When a participation's results are hidden, don't leak points/time/status —
    // render a bare "?" that links to the user's own submissions instead, same
    // as the visible submission-list link used elsewhere. Column count must
    // still match the format's normal shape so the table stays aligned.

    function hiddenProblemCell(problem, meta) {
        var url = makeSubmissionUrl(meta, problem.code);
        var inner = '?';
        if (!url) return '<td class="pending">' + inner + '</td>';
        return '<td class="pending"><a href="' + escapeHtml(url) + '">' + inner + '</a></td>';
    }

    function hiddenResultCell(meta, extraCols) {
        var url = makeAllSubmissionsUrl(meta);
        var html = url
            ? '<td class="user-points"><a href="' + escapeHtml(url) + '">?</a></td>'
            : '<td class="user-points">?</td>';
        for (var i = 0; i < extraCols; i++) {
            html += '<td class="user-penalty">?</td>';
        }
        return html;
    }
    ...
    function getRenderer(formatName) {
        ...
        return {
            ...
            resultExtraCols: renderer.resultExtraCols || 0,
    ...
    function buildUserRow(participation, problems, firstSolves, contest, renderer) {
        ...
        // Result cell(s)
        if (p.result_hidden) {
            html += hiddenResultCell(meta, renderer.resultExtraCols);
        } else {
        ...
        for (var pi = 0; pi < problems.length; pi++) {
            ...
            if (p.result_hidden) {
                html += hiddenProblemCell(prob, meta);
                continue;
            }
